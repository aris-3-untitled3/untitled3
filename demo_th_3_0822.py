...
# Initialize video capture (0 for webcam or provide video file path)
    ...
    def process_query(self,id):
        ...
        # 0     1 
        #1501 1502
        k=id-min(self.existing_ids) # 선택한 id 쿼리에 있는 index 계산
        while True:
            # self.data_dir='/home/hyeonuk/python/Person_reID_baseline_pytorch/Market/pytorch'     
...
if __name__ == '__main__':
    ...
    if new_person:    
        ...
        detector.save_gallery2(new_person,id)
        re_id_thread=threading.Thread(target=re_id.run)
        detection_thread = threading.Thread(target=detector.detect_query)
        query_thread = threading.Thread(target=detector.process_query,args=(id,)) # n번째 사진
