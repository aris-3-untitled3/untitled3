...
class RobotMain(object):
    ...
    def trash(self, aruco_x, aruco_y):
        ...
        time.sleep(1)
        if aruco_x <= 0:
            code = self._arm.set_position(*[aruco_x, aruco_y, 463.4, -52.1, 89.1, 126.1], speed=self._tcp_speed,    # z값 높게 이동
                                        mvacc=self._tcp_acc, radius=20.0, wait=True)
            if not self._check_code(code, 'set_position'):
                return
            time.sleep(1)
            code = self._arm.set_position(*[aruco_x, aruco_y, 463.4, 43.8, -88.5, -46.2], speed=self._tcp_speed_gripper,    # gripper 180도 회전
                                        mvacc=self._tcp_acc, radius=20.0, wait=True)
            if not self._check_code(code, 'set_position'):
                return
            time.sleep(2.5)
